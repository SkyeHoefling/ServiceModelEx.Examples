...
namespace ServiceModelEx.ServiceFabric.Services.Remoting.Client
{
   public static class ServiceProxy
   {
      ...
      public static I Create<I>(Uri serviceAddress,string listenerName = null) where I : class,IService
      {
      ...
      static I Create<I>(string applicationName,string serviceName) where I : class,IService
      {
         ...
         try
         {
            ServiceContext context = new ServiceContext {ServiceName = new Uri("fabric:/" + applicationName + "/" + serviceName) };
